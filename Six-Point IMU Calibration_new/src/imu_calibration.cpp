...
void calculate_gains(TUMBLE_Data* tumbl_d)
{
        ...
        for(int i = 0; i < 3; i++)
        {
                for (int j = 0; j < 3; j++)
                {
                        tumbl_d->gainMatrix[j][i] = (tumbl_d->measuredAcceleration[i][2*j]
                                                   - tumbl_d->axisOffset[i]
                                                   - tumbl_d->measuredAcceleration[i][2*j+1]
                                                   + tumbl_d->axisOffset[i])/2;
                }
        }
}
...
void print_tumble_outputs(TUMBLE_Data* tumbl_d)
{
        ...
        Serial.println("Gain matrix:");
        for(int i = 0; i < 3; i++)
        {
                for(int j = 0; j < 3; j++)
                {
                        Serial.print(tumbl_d->gainMatrix[i][j]);
                        Serial.print("  ");
                }
                Serial.print("\n");
        }
